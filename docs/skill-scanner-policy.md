...
| `analyzability_medium_risk`       | 80      | 70      | **40 (below default)** | Lowered from 70 to reflect catalog reality with rich reference/demo assets (including inline media examples) while keeping sub-40% coverage as HIGH risk.                    |
...
- `analyzability_medium_risk` at 40 (well below default's 70) — treats low analyzability as MEDIUM down to 40 while preserving HIGH severity for skills below 40%
