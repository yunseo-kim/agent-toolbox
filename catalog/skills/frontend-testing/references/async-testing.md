...
vi.runOnlyPendingTimers()     // Run only pending (not newly created)
vi.advanceTimersByTime(1000)  // Advance by specific time
...
```typescript
// Bad - if first fails, won't know about second
await waitFor(() => {
  expect(screen.getByText('Title')).toBeInTheDocument()
  expect(screen.getByText('Description')).toBeInTheDocument()
})

// Good - separate findBy calls
